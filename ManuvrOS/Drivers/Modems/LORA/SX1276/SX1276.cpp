...
*/
const uint8_t SX1276::_get_reg_addr(SX127xRegister r) {
...
const SX127xRegister SX1276::_reg_id_from_addr(uint8_t addr) {
  switch (0x7F & addr) {
...
uint SX1276::consoleGetCmds(ConsoleCommand** ptr) {
  *ptr = (ConsoleCommand*) &console_cmds[0];
  return sizeof(console_cmds) / sizeof(ConsoleCommand);
}


void SX1276::consoleCmdProc(StringBuilder* input) {
    ...
    case 'i':
      switch (temp_int) {
        ...
        case 0:
          ...
          break;
        case 1:
          local_log.concatf("\tReset: %u\tCS:    %u\n", _opts.reset_pin, _opts.cs_pin);
          local_log.concatf("\tD0:    %u\tD1:    %u\n", _opts.d0_pin, _opts.d1_pin);
          local_log.concatf("\tD2:    %u\n", _opts.d2_pin);
          break;
      }
      break;
